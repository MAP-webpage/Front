...
const Secretboardpage = () => {
  ...
  const fetchPosts = async () => {
    let url = `/secret-board/list?searchKeyword=${searchQuery}&page=${page}&size=12`; // 페이지네이션 적용
  ...
  const handleViewPost = async (boardId) => {
    try {
      await apiClient.get(`/secret-board/view/${boardId}`);

      const response = await apiClient.get(`/secret-board/view/${boardId}`);
  ...
  return (
    <div className="secretboard-container">
      <Header />
      ...
      {/* 게시글 목록 */}
      <div className="secretboard-grid">
        {posts.length === 0 ? (
          ...
          posts.map((post) => (
            <motion.div key={post.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
              <Card className="secretboard-post-card">
                <CardContent>
                  <div className="clickable-post" onClick={() => handleViewPost(post.id)}>
                    <h2 className="secretboard-post-title">{post.title}</h2>
                  </div>
                  <p className="secretboard-post-meta">
